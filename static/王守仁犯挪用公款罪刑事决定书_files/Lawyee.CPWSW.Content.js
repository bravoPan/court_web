$(function () {
    ...
    if (typeof (WebsiteLog) != "undefined") {
        ...
        WebsiteLog.Config.Mode = "DataLog,WebLog";
        WebsiteLog.Init();
    }
  //try{ $("#divtdcApp").MobileAppQrCode();}catch{}
...
var Content = {
    ...
    Content: {
        InitPlugins: function () {
            ...
            //二维码
            var jsonData = { TDCStyle: { "tdcWidth": 200, "tdcHeight": 200 }, DirectoryData: [] };

            if ($("#hidRequireLogin").val() == "1") {
                //用户访问频次过多，需要用户填写验证码,概要信息和目录等信息不显示
                if ($("#divTool_Summary").length > 0) {
                    $("#divTool_Summary").ContentSummary({ data: dirData });
                }
                if ($("#divTool_Share").length > 0) {
                    $("#divTool_Share").ContentFunction({ data: jsonData });
                }
            } else {
                ...
                //初始化分享控件
                jsonData = { TDCStyle: { "tdcWidth": 240, "tdcHeight": 240} };
                if ($("#divTool_Share").length > 0) {
                    $("#divTool_Share").ContentFunction({ data: jsonData });
                }
                if (docId != "") {
...
//                    });
                    //初始化目录控件
                    jsonData = {};
                    if ($("#divTool_Dir").length > 0) {
                        $("#divTool_Dir").ContentDirectory({ data: jsonData });
                    }
                }
            }
        },
